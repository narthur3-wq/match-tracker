  ...
  <div class="kpis">
    ...
    <div class="kpi card">
      ...
      <div class="kpi-vals">
        ...
        <span><span>{toTable.total.gain} win</span> / <span class="loss">{toTable.total.loss}</span> loss</span>  
        </div>
       ...
       <div class="block-h">Turnovers — H1 / H2 / Total (Win/Loss)</div> 
        ...
        <thead><tr><th></th><th>Win</th><th class="loss">Loss</th><th>%</th><th>F</th><th>U</th></tr></thead>  
         ...
         <tr><td>H1</td><td>{toTable.h1.gain}</td><td class="loss">{toTable.h1.loss}</td><td>{toTable.h1.pct}%</td><td>{toTable.h1.forced}</td><td>{toTable.h1.unforced}</td></tr>
            ...
            <tr class="total"><td>Total</td><td>{toTable.total.gain}</td><td class="loss">{toTable.total.loss}</td><td>{toTable.total.pct}%</td><td>{toTable.total.forced}</td><td>{toTable.total.unforced}</td></tr>          </tbody>
        </table>
